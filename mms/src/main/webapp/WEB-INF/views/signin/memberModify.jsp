...
<script>
	...
	$(function() {
		...
		$('input:text[name=birth]').val(today);
		$('input:text[name=birth]').datepicker({
			changeMonth: true,
			changeYear: true
		});
	...
	function submitChk() {
		...
		var phone = $('select[name=countryCode]').val() + "-" + $('input:text[name=phone]').val();
		var phone2 = $('input:text[name=phone]').val();
		...
		if(userpwd == "") {
			...
			fnMove($('input:text[name=username]'));
			return;
		}
		else if(phone2 == "") {
			alert("전화번호 입력하세요");
			$('input:text[name=phone]').focus();
			fnMove($('input:text[name=phone]'));
			return;
		}
		else if( isNaN(phone2) ) {
			alert("숫자만 입력해 주세요");
			$('input:text[name=phone]').select();
			fnMove($('input:text[name=phone]'));
			return;
		}
		else if(height == "") {
			...
			fnMove($('input:text[name=height]'));
			return;
		}
		else if( isNaN(height) ) {
			alert("숫자만 입력해 주세요");
			$('input:text[name=height]').select();
			fnMove($('input:text[name=height]'));
			return;
		}
		else if(weight == "") {
			...
			fnMove($('input:text[name=weight]'));
			return;
		}
		else if( isNaN(weight) ) {
			alert("숫자만 입력해 주세요");
			$('input:text[name=weight]').select();
			fnMove($('input:text[name=weight]'));
			return;
		}
		else if(address == "") {
			...
			fnMove($('input:text[name=address]'));
			return;
		}
		
		// 날짜를 DB형식에 맞게 변경(모든 에러체크가 끝나면 진행)
		
		var dateTmp = $('input:text[name=birth]').val($.datepicker.formatDate('yy-MM-dd', new Date())).val();		
		var dateArr = dateTmp.split('-');
		dateArr[1] = (new Date( $.datepicker.formatDate('yy-MM-dd', new Date(birth)))).getMonth()+1;
		
		birth = dateArr[0] + "-" + dateArr[1] + "-" + dateArr[2];
		$('input:text[name=birth]').val(birth);
		...
		<div class="container">
			...
			<div class="col-lg-4 col-md-6 col-sm-8">
				<div class="row loginbox">
						...
						<div class="form-control" align="center">
							<select name="countryCode" class="countryCode">
								...
								<option data-countryCode="KP" value="850">Korea North
									(+850)</option>
								<option data-countryCode="KR" value="82" Selected>Korea
									South (+82)</option>
								...
								<option data-countryCode="ZW" value="263">Zimbabwe
									(+263)</option>
							</select> &nbsp;<input name="phone" class="phone" type="text" placeholder="phone"
								size="20">
						</div>
					</div>
					<div class="col-lg-12 col-md-12 col-sm-12">
					...
					<div class="col-lg-12  col-md-12 col-sm-12">
						<a href="#" id="sumitBtn" class="btn  submitButton">Comfirm </a> <a
							href="${pageContext.request.contextPath}"
							class="btn  submitButton">Home </a>
					</div>
				</div>
				<br> <br>
